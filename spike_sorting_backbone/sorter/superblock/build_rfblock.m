...
% a tank.
%
% [SUPERBLOCKS, RFS, MRF] = BUILD_RFBLOCK(PATH, SAVE_DIR) ask user whether to
% detect RFs automatically via a pop-up dialog. If user confirms, no further
% interaction is required. Otherwise, another pop-up will appear asking the user
% to enter RF details manually. The entered data will be validated. If there is
% an error, this function terminates and the outputs will be empty.
%
% Note that no popups will occur if SAVE_DIR already has superblock data for the
% tank located at PATH. See below for details.
%
% [SUPERBLOCKS, RFS, MRF] = BUILD_RFBLOCK(PATH, SAVE_DIR, RFS_USER) providing a
% non-empty value for RFS_USER replaces automatic detection with  manual RF
% input. All dialogs are suppressed. If the entered data is invalid, this
% function terminates and the outputs will be empty. This option is planned for
% use with a GUI.
%           
...
% MRF           boolean. True if RFS has blocks that have multiple RFs. If
...
%EmilyD change here, un comment to return to normal
...
%             fprintf('User declined automatic RF detection. Exiting.\n');
            fprintf('Accepting manual RFs\n');
            
            rfs_string = get_rfs_from_user();
            
            if isempty(rfs_string)
                warning('User canceled manual input. Exiting.');
                return
            end
            
            try
                rfs = parse_rfs(rfs_string, path);
            catch
                warning('Problem in parsing RFs. Exiting.');
                return
            end
            
            if ~isValidRFcell(rfs, nBlocks)
                warning('Invalid user specified RF cell.');
                fprintf('Input: %s\n', rfs_string);
                return
            end
...
% Otherwise, status is False.
    ...
    answer = questdlg(prompt, name, 'Yes', 'No', 'Yes');
