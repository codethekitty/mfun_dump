...
%     end
	...
	i=1; % N_superblock will always equals 1
        ...
        sst_sb_dir = fullfile(sst_dir, sprintf('superblock_rf%d', first_block));
        fig_sb_dir = fullfile(fig_dir, sprintf('superblock_rf%d', first_block));
...
%             make_3d(outliers, feature, fig_sb_dir, iter_count, true);
            make_3d_combined(chan_tbl, feature, fig_sb_dir, iter_count);
            disp('3D plot saved.')
            toc
%             warning('on', 'all');
...
%             sst stuff
            sst = superspiketrain(tank_path, blocks, ch, 0, 1);
