function [] = sorter_c_sb(superblocks, feature, algo, ...
                                             tank_info, data_dir, K)
% CW note on this upgraded version: 
% - K is an additional input: number of clusters
% - superblocks: constructed by BUILD_SB for simple construction
% - iteration and metadata is disabled, but the data_dir is specified with 
% names of feature and sorting algorithm functions.
% - shift option = 'max' and shift amount = 5 is set as default. Users may 
% only change it in the function itself.
% - SUPERSPIKETRAIN and SUPERSPIKETRAIN_MULTI for sst processing is 
% updated. Make sure the correct sst core codes are linked before sorting.
% - MAKE_3D_COMBINED is a new function that plots sorted and unsorted clusters 
% in the same plot.
% - unit_0 is now multiunit sst (before it was just unsorted clusters)
% - disabled part pruning. The old method resulted in discrepancy in
% NTrials.
% - added checkpoints.
%
%
%
%
% % The following message was from the old version by DanielX.
%
...
%     iter_count = run_count(fig_dir);
    iter_count=0; %disabled iter feature;
%     if iter_count > 0
...
%     end
	...
	i=1; % N_superblock will always equals 1
        ...
        if length(tank)==1
            ...
            for bl=1:length(tank)
                bls=unique(sb.block(sb.tank==tank(bl)));
                bls=[bls,ones(length(bls),1)*bl];
                bl_matrix=[bl_matrix;bls];
            end
            blocks=transpose(bl_matrix);
            first_block=blocks(1,1);
        end

        sb_dir = fullfile(tank_dir, sprintf('superblock_rf%d_%dclusters_%s_%s', first_block,K,func2str(feature),func2str(algo)));        
        if ~exist(sb_dir, 'dir')
        ...
        checkpoint_path = fullfile(sb_dir, 'checkpoint.mat');
        if ~exist(checkpoint_path, 'file')
           ch_start=1;
        else
           ch_start_load=load(checkpoint_path);
           ch_start=ch_start_load.ch+1;
        end
        
        
        
        for ch = ch_start:nChannels 
            ...
            shift = 5;
            aligned = align_snip(chan_tbl.waves, shift);

            disp('Waveform aligned.')
            toc
            % User K
            %warning('off', 'all');
...
%             K = 2;
            ...
            chan_tbl.sortc = find_outliers(feature(chan_tbl.waves), class,6);
            clear class
            
            disp('Features and outliers identified.')
            toc
            % figures
...
%             make_3d(outliers, feature, fig_sb_dir, iter_count, true);
            make_3d_combined(chan_tbl, feature, sb_dir, iter_count,20000);
            disp('3D plot saved.')
            toc
%             warning('on', 'all');
...
%            make_allspikes(outliers, fig_sb_dir, iter_count);
            ...
            make_units(real, sb_dir, iter_count);
            disp('Sorted waveform plotted.')
            toc
...
%             sst stuff
            if length(tank)==1
                ...
                sst = superspiketrain(char(tank_path_str), reshape(blocks,1,numel(blocks)), ch, 0);
            else
                sst = superspiketrain_multi(tank_path, blocks, ch, 0);
            end
...
%                 
%                 if length(parts) == 1    % block has unique(FInd)==1; keep all
%                     continue
%                 end
%                 
%                 epoc_blk_idx = double(sst.Epocs.Values.Block) == block_num;
%                 
...
%             end
            ...
            %For each identified cluster, create an SST
...
%                 warning('on', 'all');
                ...
                sst_copy.SortCodeType = sprintf('%s_%s',func2str(feature),func2str(algo));
                end
...
%                 end
                ...
                sst_copy_path = fullfile(sb_dir, sst_filename);
            ...
            disp(sprintf('Channel %d sorted.',ch))
            
            save(checkpoint_path,'ch');
