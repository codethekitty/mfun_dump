classdef superspiketrain_dx
    ...
    %
    % If all inputs are left blank, superspiketrain_dx will default to reading
    ...
    %   var2 = 'timing'
    %
    %   SST = superspiketrain_dx(tankname,blocklist,channel,unit,[var1,var2])
    %
    ...
    methods
        ...
        function [obj, varargout] = superspiketrain_dx(t,b,ch,un,sb,varargin)
            ...
            legacy_tanks = 2:7;
            for idx = 1:numel(legacy_tanks)
               
                if strfind(t,['NIHL00' num2str(legacy_tanks(idx))])
                    disp('WARNING: Legacy Tank Detected. Proceed with Caution');
                    idx = numel(legacy_tanks);
                end
            ...
            %Read epoc names from first block
            TT=obj.GetBlock(TT,1);
            obj.EpocNames{1,1}=TT.GetEpocCode(0); i=1;
            while ~isempty(obj.EpocNames{i,1})
                i=i+1;
                obj.EpocNames{i,1}=TT.GetEpocCode(i-1);
            end
            obj.EpocNames(end)=[];
            ...
            for k=1:length(obj.Block)
                ...
                for j=1:length(obj.EpocNames)
                    ...
                    if strcmp(obj.EpocNames(j),'Swep') 
...
%% Old strategy for time stamp calculation and assignment
%This method is too slow for very high trial counts
%
...
%             fprintf(['Spike train from Unit: ' num2str(obj.Unit) ' of Channel ' num2str(obj.Channel) ' of %s.\n'],tanker)
        ...
        function [Output] = SortedEpocs(obj,EpocType,varargin)
            % This function returns a unique sorted list of the values 
            % associated with an Epoc title, in ascending order.
            %
            % SortedEpocs('EpocType',[...]) returns selected values of
            % sorted list
            ...
            %Requests the specific epoc values and creates a unique sorted
            %list of values
        ...
        function [trials varargout]=TrialSelect(obj,varargin) %#ok<STOUT>
            ...
            if isfield(In,'block')
                if isnan(In.block) % If user supplies nan for the block number, then return nan in trials.
                    trials=nan;
                    return;
                end
                BlockList = ismember(obj.Epocs.Values.Block,nominal(In.block));
                In = rmfield(In,'block');
            else
                BlockList = ones(length(obj.Indices.Trials),1);
            end
            
            if isfield(In,'casefile')
                FileList = ismember(obj.Indices.CaseFiles,In.casefile);
                In = rmfield(In,'casefile');
            else
                FileList = ones(length(obj.Indices.Trials),1);
            end
            
            EpocNamesL=fieldnames(In);
            
            selection_inds = ones(length(obj.Indices.Trials),1);
            for i = 1:length(EpocNamesL)
                if (ismember(EpocNamesL{i},obj.EpocNames()) && (isnumeric((In.(EpocNamesL{i}))) || isempty((In.(EpocNamesL{i})))))
                    
                    if isempty(In.(EpocNamesL{i}))
                        In.(EpocNamesL{i})=unique(obj.Epocs.Values.(EpocNamesL{i}));
                    end
                    selection_inds(:,i) = ismember(obj.Epocs.Values.(EpocNamesL{i}),In.(EpocNamesL{i})); 
                else
                    derp = char(EpocNamesL{i});
                    fprintf('Warning: parameter Epoc ''%s'' does not exist and is ignored.\n', derp);
                end
            end
            
            selection_inds=all(selection_inds,2);

            selection_inds = (selection_inds & BlockList) & FileList;
            
            %EpocsL.Values=obj.Epocs.Values(selection_inds,:);
            
            %stats=grpstats(EpocsL.Values,EpocNamesL,{'numel'});
            %varargout{:,:}=stats(:,1:1+length(EpocNamesL));
            trials=obj.Indices.Trials(selection_inds);
            
            % If there are no trials matching the requested cases, then
            % return nan.
            if isempty(trials)
                trials=nan;
            end
        ...
        function [data x y ah] = SpikeIntensity(obj,EpocType1,EpocType2)
            ...
            load('\\khri-ses.adsroot.itcs.umich.edu\ses\3Shared\Matlab Scripts\Superspiketrain\RFColorMap.mat');
