...
%     elseif post < 0
%         error('post value must be positive');
%     end
...
%     end
    ...
    spikes2 = zeros(numSpikes, width + 4);
        ...
        spike_seg=spikes1(:, (pre + 2):(pre + 2 * shift + 1));
    
    if strcmpi(thr,'max')
        [~,iaux]=max(spike_seg,[],2);
    elseif strcmpi(thr,'min')
        [~,iaux]=min(spike_seg,[],2);
    end
%         [~,iaux]=max(abs(spike_seg),[],2);
        
    for i = 1:numSpikes
        if iaux(i) > 1
            spikes2(i, :) = spikes1(i, (iaux(i) - 1):(iaux(i) + width + 2));
        else
            spikes2(i, :) = spikes1(i, iaux(i):(iaux(i) + width + 3));
        end
        % speed this up after validation.
    end
